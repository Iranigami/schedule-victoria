...
export default function CalendarInput({
  ...
  useEffect(() => {
    document
      .getElementById("day")!
      .scrollTo(0, (Number(selected.day) + 4) * 40);
    document
      .getElementById("month")!
      .scrollTo(0, (Number(selected.month) - 3) * 40);
    document
      .getElementById("year")!
      .scrollTo(0, (Number(selected.year) - 2000) * 40);
  }, []);
      ...
      <div className="w-[752px] h-[382px] rounded-[24px] bg-[#FAFAFA] mx-auto p-[24px]">
        <div className="flex justify-between items-start w-[704px] h-[40px] mx-auto border-b-[2px] border-[#F0F0F0]">
          ...
          <div className="font-medium text-[18px] text-orange leading-[100%]">
            {moment(`${selectedMonth}.${selectedDay}.${selectedYear}`).format(
              "DD MMMM YYYY",
            )}
          </div>
        </div>
        <div className="mt-[28px] w-[316px] h-[200px] relative mx-auto ">
          ...
          <div className="w-full h-full flex justify-center pr-[32px] absolute">
              ...
              <div hidden={false}>
                {[25, 26, 27, 28, 29, 30, 31].map((day, index: number) => (
                  <div
                    key={index}
                    hidden={false}
                    className="w-full h-[24px] text-[20px] leading-[100%] font-medium mt-[16px]"
                  >
                    {day}
                  </div>
                ))}
            ...
            <div
              id="month"
              className="w-[98px] mr-[32px] h-full overflow-y-scroll overflow-x-hidden text-black hide-scroll text-center"
              onScrollEnd={() => {
